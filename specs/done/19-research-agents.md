...
`prompts/skills/research.md` replaces the old `web-search` and `reference-researcher`
skills. Six sections: research workflow, source evaluation, citation discipline, web
fetch modes, query crafting, and deep research escalation.
...
Markdown files with TOML frontmatter (`+++` delimiters) in `$WORKSPACE/agents/`. Parsed
by `src/agents/definition.rs`. Fields:
...
Background agent execution manager. Maintains a `HashMap<String, AgentHandle>` of
running agents. Each agent:
...
`bootstrap_workspace` creates `agents/` directory and installs default agent definitions
via `install_default_agents()`.

## Deep Research Agent

`prompts/agents/deep-research.md` — the first default agent. Tools: `knowledge_search`,
`web_search`, `web_fetch`, `read_file`, `todo`. Max 40 iterations.

Methodology: mandatory TODO planning, broad search (2-3 angles), deep read (3-8 full
pages via web_fetch), cross-referencing, targeted follow-up. Structured output format
with summary, findings by sub-question, ranked sources, and uncertainties.
