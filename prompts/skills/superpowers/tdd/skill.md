...
fn retries_failed_operations_3_times() {
    ...
    let operation = || {
        ...
        if n < 3 { Err(anyhow!("fail")) } else { Ok("success") }
    };

    let result = retry_operation(operation).unwrap();
...
#[test]
fn retry_works() {
    let mock = MockOperation::new()
        .fail_times(2)
        .then_return("success");
    retry_operation(&mock).unwrap();
...
The "waste" is keeping code you can't trust. Working code without real tests is
technical debt.
...
Tests-first force edge case discovery before implementing. Tests-after verify you
remembered everything (you didn't).
...
fn rejects_empty_email() {
    let result = submit_form(FormData { email: "" });
...
```bash
$ just test
FAIL: assertion failed: expected "Email required", got Ok(...)
```

**GREEN**

```rust
fn submit_form(data: FormData) -> Result<Response, FormError> {
    if data.email.trim().is_empty() {
        return Err(FormError::EmailRequired);
    }
    // ...
}
```
...
Bug found? Write failing test reproducing it. Follow TDD cycle. Test proves fix and
prevents regression.
...
When adding mocks or test utilities, watch out for common pitfalls:
