...
description: GHOST's knowledge system — notes, references, diary entries with graph edges and semantic search.
---

import { FileTree, Tabs, TabItem } from "@astrojs/starlight/components";

GHOST's knowledge system stores and connects information using three types of
entries, backed by SurrealDB with graph edges and embedding-based semantic
search.
...
Atomic knowledge entries with structured metadata. Your GHOST creates these
during reflection and conversation.
...
Rust uses `Result<T, E>` for recoverable errors and `panic!` for
unrecoverable ones.
...
Notes connect to each other through wiki links with optional typed
relationships:
...
These links create edges in the graph database, enabling traversal and
discovery. Unresolved links automatically create **stub notes** that your
GHOST can flesh out later.
...
Results are merged and ranked. Search covers notes, references, and diary
entries.
