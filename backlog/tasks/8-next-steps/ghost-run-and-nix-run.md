...
The current `agent` tool has actions: `start`, `status`, `show`, `cancel`. The `run`
tool keeps the same actions but handles both agents and scripts:
...
The scheduler watches `schedule.toml` for changes (same debounced file watcher pattern
as the current `crontab.lua` watcher). On change, it reloads all entries.
...
- **GHOST-authored scripts**: the GHOST runs them after writing. No test framework in
  the workspace — the development loop is write → run → check → fix.
