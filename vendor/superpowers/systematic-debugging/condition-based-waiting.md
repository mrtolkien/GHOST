...
where tests pass on fast machines but fail under load or in CI.
...
with domain-specific helpers (`waitForEvent`, `waitForEventCount`, `waitForEventMatch`)
...
**❌ Stale data:** Cache state before loop **✅ Fix:** Call getter inside loop for fresh
data
