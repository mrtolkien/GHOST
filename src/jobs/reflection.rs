...
Append to today's diary using file_edit on `$WORKSPACE/diary/YYYY-MM-DD.md`. \
Create the file with write_file if it doesn't exist. Each entry is a timestamped \
bullet point:
...
1. **Plan**: Use `todo(action=\"plan\", items=[...])` to create a TODO list of \
   knowledge operations: notes to create/update, web cache to curate, diary \
   entries, identity updates
2. **Execute**: Work through the TODO list. Create/update notes (note_write), \
   curate web cache (reference_manage), write diary (file_edit), update identity \
   (file_edit). Mark items done with `todo(action=\"batch_update\", \
   updates=[...])` — prefer batch_update over individual update calls.
3. **Handoff**: Your final message becomes the handoff note for the next \
   reflection. Use `todo(action=\"batch_update\")` to mark remaining items done \
   or skipped before finishing.";
